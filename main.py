        ...
        try:
            ...
            try:
                ...
                cur.execute("""
                    ...
                    ADD COLUMN IF NOT EXISTS allowed_fertilizer_types TEXT,
                    ADD COLUMN IF NOT EXISTS allowed_recommended_times TEXT,
                    ADD COLUMN IF NOT EXISTS allowed_flowering_types TEXT,
                    ADD COLUMN IF NOT EXISTS preferred_terpenes TEXT,
                    ADD COLUMN IF NOT EXISTS allowed_status TEXT;
                """)
                ...
                cur.execute("""
                    ...
                    ADD COLUMN IF NOT EXISTS max_cbd DECIMAL(5,2);
                """)
                ...
                cur.execute("""
                    ...
                    ADD COLUMN IF NOT EXISTS image_weight INTEGER DEFAULT 0,
                    ADD COLUMN IF NOT EXISTS cert_weight INTEGER DEFAULT 0;
                """)
                ...
                cur.execute("""
                    ...
                    ADD COLUMN IF NOT EXISTS require_min_reviews BOOLEAN DEFAULT FALSE,
                    ADD COLUMN IF NOT EXISTS min_review_count INTEGER DEFAULT 1;
                """)
    ...
    if conn:
        try:
            ...
            bud_data = {
                ...
                'top_terpenes_1_percentage': float(result[12]) if result[12] else None,
                'top_terpenes_2_percentage': float(result[13]) if result[13] else None,
                'top_terpenes_3_percentage': float(result[14]) if result[14] else None,
                'mental_effects_positive': result[15],
                'mental_effects_negative': result[16],
                'physical_effects_positive': result[17],
                'physical_effects_negative': result[18],
                'recommended_time': result[19],
                'grow_method': result[20],
                'harvest_date': result[21].strftime('%Y-%m-%d') if result[21] else None,
                'batch_number': result[22],
                'grower_id': result[23],
                'grower_license_verified': result[24],
                'fertilizer_type': result[25],
                'flowering_type': result[26],
                'image_1_url': result[27],
                'image_2_url': result[28],
                'image_3_url': result[29],
                'image_4_url': result[30],
                'created_at': result[31].strftime('%Y-%m-%d %H:%M:%S') if result[31] else None,
                'updated_at': result[32].strftime('%Y-%m-%d %H:%M:%S') if result[32] else None,
                'created_by': result[33],
                'lab_test_name': result[34],
                'test_type': result[35],
                'certificate_image_1_url': result[36],
                'certificate_image_2_url': result[37],
                'certificate_image_3_url': result[38],
                'certificate_image_4_url': result[39]
            }
    ...
    try:
        ...
        cur.execute(f"""
            SELECT b.id, b.strain_name_en, b.strain_name_th, b.breeder, b.strain_type,
                   b.thc_percentage, b.cbd_percentage, b.grade, b.aroma_flavor,
                   b.top_terpenes_1, b.top_terpenes_2, b.top_terpenes_3,
                   b.top_terpenes_1_percentage, b.top_terpenes_2_percentage, b.top_terpenes_3_percentage,
                   b.mental_effects_positive, b.mental_effects_negative,
                   b.physical_effects_positive, b.physical_effects_negative,
                   b.recommended_time, b.grow_method, b.harvest_date, b.batch_number,
                   b.grower_id, b.grower_license_verified, b.fertilizer_type, 
                   ...
                   COALESCE(u_creator.profile_image_url, u_grower.profile_image_url) as creator_profile_image,
        ...
        bud_info = {
            ...
            'recommended_time': result[19],
            'grow_method': result[20],
            'harvest_date': format_date_safely(result[21], 'date'),
            'batch_number': result[22],
            'grower_id': result[23],
            'grower_license_verified': result[24],
            'fertilizer_type': result[25],
            ...
            'image_3_url': result[29],
            'image_4_url': result[30],
            'created_at': format_date_safely(result[31], 'datetime'),
            'updated_at': format_date_safely(result[32], 'datetime'),
            'created_by': result[33],
            'lab_test_name': result[34],
            'test_type': result[35],
...
@app.route('/api/admin/preview_eligible_buds', methods=['POST'])
def preview_eligible_buds():
    """Preview buds that match the activity criteria"""
    if not is_admin():
        return jsonify({'error': 'Unauthorized'}), 401
    ...
    if conn:
        try:
            ...
            if criteria.get('min_thc') is not None:
                where_conditions.append("b.thc_percentage >= %(thc_min)s")
                params['thc_min'] = criteria['min_thc']

            if criteria.get('max_thc') is not None:
                where_conditions.append("b.thc_percentage <= %(max_thc)s")
                params['max_thc'] = criteria['max_thc']

            if criteria.get('min_cbd') is not None:
                where_conditions.append("b.cbd_percentage >= %(cbd_min)s")
                params['cbd_min'] = criteria['min_cbd']

            if criteria.get('max_cbd') is not None:
                where_conditions.append("b.cbd_percentage <= %(max_cbd)s")
                params['max_cbd'] = criteria['max_cbd']

            # Terpenes filter
            if criteria.get('preferred_terpenes') and criteria['preferred_terpenes'] != '':
            ...
            if criteria.get('require_certificate'):
            ...
            if criteria.get('require_min_images'):
                ...
                for i in range(1, min_images + 1):
                    image_conditions.append(f"b.image_{i}_url IS NOT NULL")
                where_conditions.append(f"({' AND '.join(image_conditions)})")
            ...
            buds = []
            for row in results:
                # Apply review count filter if needed
                if criteria.get('require_min_reviews') and row[10] < criteria.get('min_review_count', 1):
                    continue
            ...
            print(f"Error in preview_eligible_buds: {e}")
            return jsonify({'error': str(e)}), 500
        finally:
            cur.close()
            return_db_connection(conn)
