...
@app.route('/admin/strains')
def admin_strains():
    """Admin strains management page"""
    if not is_authenticated():
        return redirect('/auth')
    if not is_admin():
        return redirect('/profile?no_admin=1')
    return render_template('admin_strains.html')

@app.route('/api/admin/strains')
def get_admin_strains():
    ...
    if conn:
        try:
            ...
            cur.execute("""
                SELECT id, name_th, name_en, is_popular, created_at
                FROM strain_names 
...
@app.route('/api/admin/strains', methods=['POST'])
def admin_add_strain():
    """Admin add new strain"""
    if not is_authenticated() or not is_admin():
        return jsonify({'error': 'Unauthorized'}), 401
...
@app.route('/api/admin/strains/<int:strain_id>', methods=['DELETE'])
def admin_delete_strain(strain_id):
    """Admin delete strain"""
    if not is_authenticated() or not is_admin():
        return jsonify({'error': 'Unauthorized'}), 401
    ...
    if conn:
        try:
            ...
            # Check if strain exists and get name for confirmation
            cur.execute("""
                SELECT name_en, name_th FROM strain_names WHERE id = %s
            """, (strain_id,))
            
            strain = cur.fetchone()
            if not strain:
                return jsonify({'error': 'ไม่พบสายพันธุ์ที่ต้องลบ'}), 404

            strain_name = strain[0] or strain[1] or f'ID {strain_id}'

            # Delete the strain
            cur.execute("DELETE FROM strain_names WHERE id = %s", (strain_id,))
            
            if cur.rowcount == 0:
                return jsonify({'error': 'ไม่สามารถลบสายพันธุ์ได้'}), 400
            ...
            return jsonify({
                'success': True,
                'message': f'ลบสายพันธุ์ "{strain_name}" เรียบร้อยแล้ว'
            })
